...
<nav class="min-w-full py-4 m-0 flex flex-col md:flex-row items-center flex-wrap justify-around font-serif px-6 bg-background-primary text-xl text-copy-primary">
  <div class="flex flex-row items-center">
    @if($settings->logo_location != null && $settings->logo_description != null)
      <a href="/"><img class="h-16" src="{{ secure_asset('storage/'.$settings->logo_location) }}" alt="{{ $settings->logo_description }}"></a>
    @else 
      <a href="/" class="font-bold text-3xl text-copy-secondary">Arachni<span class="text-copy-primary">CMS</span></a>
    @endif
  </div>
  <button type="button" id="menu-burger" class="relative block md:hidden">
  ...
  <button type="button" id="menu-cancel" class="relative hidden md:hidden">
    <svg version="1.1" class="fill-current w-8 h-8" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 20 20" enable-background="new 0 0 20 20" xml:space="preserve">
  ...
  <div class="menu-item hidden md:flex flex-col md:flex-row items-center justify-between flex-wrap">
    <div class="mr-4 md:mr-8 ml-4">
