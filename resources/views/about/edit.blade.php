  ...
  <form action="{{ url('about') }}" method="post" enctype="multipart/form-data">
    ...
    <div class="mb-4 flex flex-col">
      ...
      <label for="image_file" class="text-copy-primary text-sm font-bold mb-6 mt-3 py-2">
        ...
        <small class="text-copy-primary font-normal text-xs">It looks like you've already uploaded an image for your
          About Page. If you're happy with the image you already have, then you don't need to do anything.</small>
        <small class="text-copy-primary font-normal text-xs">If you decide to upload an image to replace the old one,
          providing a description for the image below is mandatory. Note that the max upload size for an image is 2MB,
          so if your image is larger than that, you will need to resize it by way of image manipulation software, such
          as GIMP.</small>
      </label>
      <input type="file" name="image_file" id="image_file"
        class="text-copy-primary bg-background-form shadow appearance-none border rounded w-full mt-0 py-2 px-3 text-copy-primary leading-tight focus:outline-none focus:shadow-outline focus:bg-background-ruthieslight @error('image_file') border-solid border-red-600 border-2 @enderror">
      ...
      <div class="mb-4">
        <label for="image_file" class="block text-copy-primary text-sm font-bold mb-3">
          Upload An Image<br class="my-2">
          <small class="text-copy-primary font-normal text-xs">It isn't required for you to upload an image to create
            your About Page, if that's your style. However, if you decide to upload an image, providing a description
            for the image below is mandatory. Note that the max upload size for an image is 2MB, so if your image is
            larger than that, you will need to resize it by way of image manipulation software, such as GIMP.</small>
        </label>
        <input type="file" name="image_file" id="image_file"
      ...
      <div class="mb-4">
        <label for="image_description" class="block text-copy-primary text-sm font-bold mb-3">
          Image Description<br class="my-2">
          <small class="text-copy-primary font-normal text-xs">The description you provide for the image is for
            situations such as disabled users utilizing screen readers or network conditions preventing image loading
            for visitors to your site.</small>
        </label>
        <input type="text" name="image_description" id="image_description"
          class="text-copy-primary bg-background-form shadow appearance-none border rounded w-full py-2 px-3 text-copy-primary leading-tight focus:outline-none focus:shadow-outline focus:bg-background-ruthieslight @error('image_description') border-solid border-red-600 border-2 @enderror"
          value="{{ old('image_description') }}">
