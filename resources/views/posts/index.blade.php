...
<link rel="stylesheet" href="{{ asset('css/dpo-fin-s1.css') }}">
@endsection
...
<div class="max-w-lg w-10/12 mt-8 mx-auto flex flex-row justify-between items-center">
  <a href="/posts/create">
    <button
      class="border bg-background-primary text-copy-secondary py-2 px-4 rounded hover:bg-background-secondary font-bold"><svg
        version="1.1" class="fill-current h-8 w-8" xmlns="http://www.w3.org/2000/svg"
  ...
  <a href="/posts-feed" class="text-copy-primary hover:text-copy-secondary inline-block">
    <svg version="1.1" class="fill-current w-10 h-10" xmlns="http://www.w3.org/2000/svg"
      xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 20 20" enable-background="new 0 0 20 20"
      xml:space="preserve">
      <path
        d="M2.4,2.4v2.367c7.086,0,12.83,5.746,12.83,12.832h2.369C17.599,9.205,10.794,2.4,2.4,2.4z M2.4,7.137v2.369  c4.469,0,8.093,3.623,8.093,8.094h2.368C12.861,11.822,8.177,7.137,2.4,7.137z M4.669,13.059c-1.254,0-2.27,1.018-2.27,2.271  s1.016,2.27,2.27,2.27s2.269-1.016,2.269-2.27S5.923,13.059,4.669,13.059z" />
...
@if(count($posts) > 0)
@guest
<aside class="flex flex-row justify-center items-center mt-8">
...
<article
  class="max-w-lg w-10/12 bg-background-primary shadow-lg rounded mx-auto px-8 pt-6 pb-8 mt-8 {{ $settings->font_pref == 'gsa' ? 'font-sans' : '' }} {{ $settings->font_pref == 'gsa' ? 'font-sans' : '' }} {{ $settings->font_pref == 'gse' ? 'font-serif' : '' }} {{ $settings->font_pref == 'gmo' ? 'font-mono' : '' }} {{ $settings->font_pref == 'asa' ? 'font-alegreya-sans' : '' }} {{ $settings->font_pref == 'ase' ? 'font-alegreya' : '' }} {{ $settings->font_pref == 'fco' ? 'font-fira-code' : '' }} {{ $settings->font_pref == 'hac' ? 'font-hack' : '' }} {{ $settings->font_pref == 'mon' ? 'font-montserrat' : '' }} {{ $settings->font_pref == 'qui' ? 'font-quicksand' : '' }}">
  <h1
    class="font-semibold text-center text-2xl text-copy-primary mb-4 {{ $settings->font_pref == 'asa' ? 'font-alegreya-sans-sc' : '' }} {{ $settings->font_pref == 'ase' ? 'font-alegreya-sc' : '' }}">
    {{ $post->title }}
  </h1>
  <dl class="my-4 cursor-default flex flex-row flex-wrap justify-around">
    ...
    <dd
      class="flex flex-row items-center align-center border w-auto bg-background-secondary text-copy-primary hover:text-copy-secondary font-semibold rounded-full py-2 px-4 text-sm mx-4 my-2 mx-auto">
  ...
  <section id="post-body" class="text-copy-primary">
    ...
    @endif
    <section class="flex flex-row justify-between mx-10">
      <dl
        class="mt-4 flex flex-row justify-center items-center text-copy-primary hover:text-copy-secondary cursor-pointer w-auto">
        <dt class="sr-hidden">Comment Count</dt>
      ...
      <a href="/posts/{{ $post->url_string }}"><button
          class="border bg-background-secondary text-copy-secondary py-2 px-4 ml-4 rounded hover:bg-background-primary font-bold h-auto"
          aria-label="Read Full Article with Title: {{ $post->title }}">Read more...</button></a>
    </section>
  </section>
</article>
@endforeach
