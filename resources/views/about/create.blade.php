...
@section('content')
  <div class="max-w-2xl w-10/12 bg-background-primary shadow-lg rounded mx-auto px-8 pt-6 pb-8 mt-8 font-sans">
    ...
    <form action="/about" method="post" enctype="multipart/form-data">
      ...
      <input type="text" name="title" id="title" required class="text-copy-primary bg-background-form shadow appearance-none border rounded w-full py-2 px-3 text-copy-primary leading-tight focus:outline-none focus:shadow-outline focus:bg-background-ruthieslight @error('title') border-solid border-red-600 border-2 @enderror" value="{{ old('title') }}" autofocus>
      </div>
      <div class="mb-4">
        <label for="about-body" class="block text-copy-primary text-sm font-bold mb-2">
          About Page Body
        </label>
        <textarea name="about_body" id="about-body" class="text-copy-primary bg-background-form shadow appearance-none border rounded w-full py-2 px-3 text-copy-primary leading-tight focus:outline-none focus:shadow-outline focus:bg-background-ruthieslight @error('body') border-solid border-red-600 border-2 @enderror" required>{{ old('about-body') }}</textarea>
      </div>
      <div class="mb-4">
