...
<div id="deletionModal"
  ...
  <div class="flex flex-row justify-end">
    <button type="button" id="cancelDelete"><svg version="1.1" class="fill-current h-8 w-8 cursor-pointer"
...
<div id="imageableDeletionModal"
  class="fixed z-30 bg-background-primary border text-copy-primary rounded mx-auto max-w-3xl w-11/12 hidden text-center py-4 px-6 container inset-x-0 h-auto my-8 md:my-16">
  <div class="flex flex-row justify-end">
    <button type="button" id="idmClose"><svg version="1.1" class="fill-current h-8 w-8 cursor-pointer"
        xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
        viewBox="0 0 20 20" enable-background="new 0 0 20 20" xml:space="preserve">
        <path
          d="M16,2H4C2.9,2,2,2.9,2,4v12c0,1.1,0.9,2,2,2h12c1.1,0,2-0.9,2-2V4C18,2.9,17.1,2,16,2z M13.061,14.789  L10,11.729l-3.061,3.06L5.21,13.061L8.271,10l-3.06-3.061L6.94,5.21L10,8.271l3.059-3.061l1.729,1.729L11.729,10l3.06,3.061  L13.061,14.789z" />
  ...
  <p class="font-bold text-2xl">
    ...
    <form action="{{ url('imageables') }}" method="post">
      ...
      <button type="submit" id="deleteImageableBtn"
        class="bg-background-secondary border text-copy-secondary hover:text-copy-primary font-bold w-5/6 rounded py-2 px-4 mt-4">Delete</button>
    </form>
  </p>
</div>
<div class="max-w-2xl w-10/12 mt-8 mx-auto flex flex-col items-start">
  <a href="/posts/{{ $post->url_string }}"
    class="border bg-background-primary text-copy-secondary py-2 px-4 rounded hover:bg-background-secondary font-bold h-auto">
...
<div class="max-w-2xl w-10/12 bg-background-primary shadow-lg rounded mx-auto px-8 pt-6 pb-8 mt-8 font-sans">
  ...
  <form action="{{ url('posts') }}" method="post">
    ...
    <div class="mb-4">
      ...
      <textarea name="summary" id="post-body"
        class="text-copy-primary bg-background-form shadow appearance-none border rounded w-full py-2 px-3 text-copy-primary leading-tight focus:outline-none focus:shadow-outline focus:bg-background-ruthieslight @error('body') border-solid border-red-600 border-2 @enderror">{{ $post->summary }}</textarea>
    </div>
    <small class="text-copy-primary">On some pages, such as the page the displays a paginated list of multiple blog
      posts, it may be unrealistic to display the whole body of the blog post. Instead, we'll display the short,
      unstyled summary you type in here. Adding a summary is not required, but advised.</small>
    <div class="mt-4 mb-4">
    ...
    <div class="relative w-full mb-4">
      <select name="is_published" id="is-published" required
        class="block appearance-none w-full bg-background-secondary text-copy-primary border px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline">
    ...
    <div class="mt-2 mb-4">
      <small class="text-copy-primary">This determines whether your post is private (and only you can see it when you're
        logged in) or public (and all visitors to your blog can read it). You can change this option later.</small>
    </div>
    <div class="flex flex-col items-start mb-4">
      ...
      <p class="mb-2 text-copy-secondary">
        <small>While this is set to true, visitors will not be able to comment on this blog post. While it is set to
          false, visitors will be able to comment on this blog post.</small>
      </p>
      <div class="inline-block relative w-full">
        <select name="comments_locked"
          class="block appearance-none w-full bg-background-secondary text-copy-primary border px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline @error('comments_locked') border-solid border-red-600 border-2 @enderror"
          value="{{ $post->comments_locked ? 'true' : 'false' }}" required>
    ...
    <button type="submit"
      class="border bg-background-secondary text-copy-secondary py-2 px-4 rounded hover:bg-background-primary font-bold w-full">
      Update Post
    </button>
  </form>
  <button type="button" id="delete"
    class="bg-orange-500 hover:bg-orange-600 text-white hover:text-gray-200 py-2 px-4 rounded font-bold w-full mt-10">
    Delete Post
  </button>
</div>
<div class="max-w-2xl w-10/12 bg-background-primary shadow-lg rounded mx-auto px-8 pt-6 pb-8 mt-8 font-sans">
  ...
  <form action="{{ url('youtubevidembeds') }}" method="post" class="mb-16">
    ...
    <button type="submit"
      class="bg-orange-500 hover:bg-orange-600 text-white hover:text-gray-200 py-2 px-4 rounded font-bold w-full">
